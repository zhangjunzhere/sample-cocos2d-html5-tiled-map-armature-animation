...
var GameLayer = cc.LayerGradient.extend({
    ...
    init: function () {
        ...
        //Tile Maps
        _tilemaps.push('asset/maps/map0.tmx');
        //_tilemaps.push('asset/maps/map1.tmx');
        //_tilemaps.push('asset/maps/map2.tmx');
        //_tilemaps.push('asset/maps/map3.tmx');
        //_tilemaps.push('asset/maps/map4.tmx');
        //_tilemaps.push('asset/maps/map5.tmx');
        _tilemaps.push('asset/maps/map6.tmx');
        _tilemaps.push('asset/maps/map7.tmx');
        _tilemaps.push('asset/maps/map8.tmx');
        _tilemaps.push('asset/maps/map9.tmx');
        //_tilemaps.push('asset/maps/map10.tmx');
        _tilemaps.push('asset/maps/mapTransition.tmx');

        _tilemapsBackground.push('asset/maps/map0background.tmx');

        _tilemapsBackground2.push('asset/maps/map0background2.tmx');
        ...
        ccs.ArmatureDataManager.getInstance().addArmatureFileInfo('asset/RunnerFinal/RunnerFinal0.png', 'asset/RunnerFinal/RunnerFinal0.plist', 'asset/RunnerFinal/RunnerFinal.ExportJson');
    ...
    initGame: function () {
        ...
        setTimeout(function (updateGame) {
            ...
            label.runAction(cc.Sequence.create(cc.FadeOut.create(1.0), cc.CallFunc.create(function (label) {
                layerSprite.removeChild(label);
            }, label)));

            layerSprite.addChild(label);

            cc.AudioEngine.getInstance().stopMusic();
            cc.AudioEngine.getInstance().playMusic('asset/audios/summerSong.mp3', true);
    ...
    updateGame: function () {
        ...
        if (localPlayer.onGround && runSoundDelay < 0 && localPlayer.getAnimation().getCurrentMovementID() != 'jump') {
            cc.AudioEngine.getInstance().playEffect('asset/audios/run.wav', false);
            runSoundDelay = 8;
        } else
            runSoundDelay -= 0.5;

        //Manda a posi��o para os outros jogadores
        if (socket)
            socket.emit('move player', {
                x: (voidNode.getPosition().x * -1) + localPlayer.getPosition().x,
                y: localPlayer.getPosition().y
            });
        ...
        if ((voidNode.getPosition().x * -1) > ((tileMapSize * loopNumber)) - 900) {
            ...
            if (mapsCount == 6) {
                ...
                //Senao, seleciona um mapa aleat�rio
        ...
        //Atualiza os corpos de acordo com a posi��o do Node
